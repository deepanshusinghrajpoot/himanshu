...
=========== GROUP BY + HAVING (1-FRAME CHEAT SHEET) ===========

WHEN TO USE
-----------
GROUP BY → when you want aggregation per group
HAVING   → when you want to filter aggregated data


lll CORE RULE ⭐ (MOST IMPORTANT)
==================================
...
lll WHERE RULE ⭐
=================
WHERE can use ONLY normal columns (no aggregates)
...
HAVING → aggregates allowed only


VALID EXAMPLES
--------------
-- Total sales per year > 1000

SELECT year, SUM(gross)
FROM movies
GROUP BY year
HAVING SUM(gross) > 1000;


-- Avg rating per director > 8
...
SELECT year, gross
FROM movies
GROUP BY year;        -- gross not grouped or aggregated ❌

SELECT year
FROM movies
WHERE SUM(gross) > 1000;  -- aggregate in WHERE ❌
